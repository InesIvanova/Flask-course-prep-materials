...
class TestComplaint(TestCase):
    ...
    def test_complaint(self, mocked_upload, mock_transaction):
        ...
        mocked_upload.assert_called_once_with(path, mock_uuid(), data["photo_extension"])
        mock_transaction.assert_called_once_with(data["amount"], f"{comp.first_name} {comp.last_name}", comp.iban, comp.id)
        complaints = ComplaintModel.query.all()
